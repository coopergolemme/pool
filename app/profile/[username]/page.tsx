...
export default function ProfilePage() {
    ...
    const chartData = useMemo(() => {
        ...
        // Track ratings as we iterate
        // We only start plotting once the profile user has played at least one game
        let currentCtxUserRating: number | undefined = undefined;
        let currentProfileRating: number | undefined = undefined;
        ...
        for (const game of sortedGames) {
            ...
            const profilePlayed = game.players.includes(username);
            const userPlayed = currentUserProfile && game.players.includes(currentUserProfile.username);

            // Update ratings based on this game's result
            if (profilePlayed) {
            ...
            if (userPlayed && currentUserProfile) {
                const h = history[currentUserProfile.username];
                if (h) {
            ...
            // We plot a point if:
            // 1. The profile has started (we don't plot before their first game)
            // 2. EITHER the profile user played OR the logged-in user played
            if (hasProfileStarted && (profilePlayed || userPlayed)) {
                // Basic metadata dependent on if profile played
                let opponent: string | undefined = undefined;
                let result: "W" | "L" | undefined = undefined;

                if (profilePlayed) {
                    opponent = game.players.find(p => p !== username);
                    result = game.winner === username ? "W" : "L";
                }

                data.push({
                    date: game.date,
                    rating: currentProfileRating!, // Safe bang because hasProfileStarted is true
                    gameIndex: gameIndex++,
                    opponent,
                    result,
                    userRating: (currentUserProfile?.username === username) ? undefined : currentCtxUserRating,
                });
            }
        }
        return data;
